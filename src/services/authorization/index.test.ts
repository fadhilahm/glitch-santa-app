...
describe('AuthorizationService', () => {
  ...
  describe('validateUser', () => {
    ...
    it('should return invalid for user without profile', async () => {
      // Arrange
      const usersWithExtra = [...mockUsers, { id: '4', username: 'no.profile', age: 7, address: 'No Profile St' }];
      const mockFetch = global.fetch as jest.MockedFunction<typeof fetch>;
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
    ...
    it('should handle API fetch errors', async () => {
      ...
      // Act & Assert
      await expect(authService.validateUser('1')).rejects.toThrow('Failed to fetch user data');
    });
